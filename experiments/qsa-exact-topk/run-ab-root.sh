...
[[ ${EUID:-$(id -u)} -eq 0 ]] || {
    echo "Run with sudo: sudo bash experiments/qsa-exact-topk/run-ab-root.sh" >&2
    exit 1
}
...
docker image inspect "$IMAGE" >/dev/null 2>&1 || {
    echo "Experimental image missing. Run experiments/qsa-exact-topk/build.sh first." >&2
    exit 1
}
...
echo "This planned outage stops the healthy production service, runs the exact-QSA-top-k"
...
read -r -p "Type RUN-QSA-EXACT-TOPK-AB to continue: " answer </dev/tty
[[ "$answer" == "RUN-QSA-EXACT-TOPK-AB" ]] || { echo "Cancelled."; exit 0; }
